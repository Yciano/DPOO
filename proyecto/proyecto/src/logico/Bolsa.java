...
public class Bolsa implements Serializable {
    ...
    public boolean registrarSolicitud(String identificador, String IDcompania, Date fecha, Requisito requisito, boolean estado) {
    ...
    public void removeUser(String cedula) {
        ...
        if (aux != null) {
            for (int i = misSolicitudes.size() - 1; i >= 0; i--) {
                Solicitud sol = misSolicitudes.get(i);
                if (sol.getUser().equals(aux)) {
                    misSolicitudes.remove(i);
                }
            }
            aux.getSolicitudes().clear();
            misUsers.remove(aux);
        }
    }
